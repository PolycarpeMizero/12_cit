...
For elastic net, it has to be continuous, not categorical.
...
Based on R2, we would choose   
  - tree_depth = 9
  - conditional_min_criterion = 0.9999454
...
Because we only see three terminal nodes, all the observations fall into these terminal nodes.
Use the predicted values, use them to get prediction on test set.
...
weather is the whole data set
weather_train is the train data set
...
```
Using the data from both the train and test set.
Limitation: The model is sensitive to the data you use to train the model,
We need the a robust model that can predict without basing on the data used to train model.
